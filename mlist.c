...
MList *ml_create(void){
    ...
    mlist->cap = HASHBUCKETS;//initial number of lists

    //allocate memory for the structure with points to all the lists
    if ((mlist->list = calloc(mlist->cap, sizeof(LList))) == NULL)
        return 0;

    //allocate memory for every specific list
    for (i=0;i<mlist->cap;i++){
...
int ml_add(MList **ml, MEntry *me){
    ...
    unsigned long hash = me_hash(me,(*ml)->cap);
    ...
    while(LISTELEMENTS == add_list->size){ //loop until all linked lists are of correct size
        ...
        new_capacity = (*ml)->cap;
        (*ml)->cap = ((*ml)->cap)*2;//double the capacity since a list is full

        //if not enough memmory when resizing -> return 0
        if((resize_list = calloc((*ml)->cap,sizeof(LList*))) == NULL)
            return 0;

        old_capacity = (*ml)->cap;//keep the old capacity
        ...
        for(i=0;i<new_capacity;i++){
            for(j=0;j<(*ml)->list[i]->size;j++){
                ...
                new_hash = me_hash(stepper_mentry,(*ml)->cap);
...
void ml_destroy(MList *ml){
    ...
    int i = 0;
    int j = 0;
    unsigned int initial_cap = ml->cap;
